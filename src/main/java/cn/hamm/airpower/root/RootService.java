...
public class RootService<E extends RootEntity<E>, R extends RootRepository<E>> {
    ...
    public List<E> getList(QueryRequest<E> queryRequest) {
        ...
        return repository.findAll(createSpecification(queryRequest), createSort(queryRequest));
    }
    ...
    private Sort createSort(QueryRequest<E> queryRequest) {
        ...
        if (Objects.nonNull(queryRequest.getSort()) && !"".equalsIgnoreCase(queryRequest.getSort().getField())) {
            // 如果传入了Sort和字段
