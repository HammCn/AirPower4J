...
import static cn.hamm.airpower.core.exception.ServiceError.WEBSOCKET_ERROR;
...
@Component
@Slf4j
public abstract class AbstractWebSocketHandler extends TextWebSocketHandler implements MessageListener {
    /**
     * <h3>订阅全频道</h3>
     */
     ...
     */
    @Override
    protected final void handleTextMessage(@NotNull WebSocketSession session, @NotNull TextMessage textMessage) {
    ...
    public final void afterConnectionEstablished(@NotNull WebSocketSession session) {
        ...
        AccessTokenUtil.VerifiedToken verifiedToken = AccessTokenUtil.create()
                .verify(accessToken, getAccessTokenSecret());
     ...
     * @param session 会话
     */
    protected void afterConnectSuccess(@NotNull WebSocketSession session) {
     ...
     * @param session 连接
     */
    private void onChannelMessage(@NotNull String message, @NotNull WebSocketSession session) {
        try {
     ...
     * @param userId  用户 {@code ID}
     */
    protected void afterDisconnect(@NotNull WebSocketSession session, @Nullable Long userId) {
